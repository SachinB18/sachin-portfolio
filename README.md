# Personal Portfolio Website

This repository contains my personal portfolio website built to showcase my
projects, skills, and professional profile.

## 🌐 Live Website
🔗 [https://SachinB18.github.io/<repo-name>/](https://sachinb18.github.io/sachin-portfolio/)

## 👨‍💻 About Me
I am a Computer Engineering student and Java & Backend Developer, open to freelance
work, internships, and project-based collaborations. I enjoy building real-world
applications, automation tools, and data-driven systems.

## 🛠️ Tech Stack
- HTML5
- CSS3
- JavaScript
- GitHub Pages

## 📂 Featured Projects
- **AQI Prediction System (ML + Flask)**
- **Hospital Management System (Java, JDBC, MySQL)**
- **CSV to Excel Automation Tool (Python)**
- **Automatic Timetable Generator**
- **Personal Portfolio Website**

## 📄 Resume
My resume is available directly on the website:
🔗 Sachin_Bhabad_Resume.pdf

## 📬 Contact
- Email: sachinbhabad8181@gmail.com
- GitHub: https://github.com/SachinB18
- LinkedIn: https://www.linkedin.com/in/sachin-bhabad-19a00a333

---
⭐ Feel free to explore the projects and connect with me.
